...
import pytest
import tracklab
from tracklab.sdk.integration_utils.auto_logging import AutologAPI, PatchAPI
...
def test_autolog_api(mock_autolog_api):
    ...
    tracklab.run.log.assert_called_with({"generated_text": "Hello World!"})
    ...
    # check that tracklab.run.log was called twice and with the correct arguments
    assert tracklab.run.log.call_count == 2
    tracklab.run.log.assert_called_with({"generated_text": "YES!"})
