...
def test_autoresume_first_run_saves():
    with pytest.raises(AutoResumeTestError):
        with tracklab.init(mode="offline", resume="auto") as run:
...
def test_autoresume_second_run_loads():
    with pytest.raises(AutoResumeTestError):
        with tracklab.init(mode="offline", resume="auto") as run1:
            raise AutoResumeTestError()
    with pytest.raises(AutoResumeTestError):
        with tracklab.init(mode="offline", resume="auto") as run2:
            raise AutoResumeTestError()
...
def test_explicit_id_overrides_autoresume(mock_wandb_log):
    with pytest.raises(AutoResumeTestError):
        with tracklab.init(mode="offline", resume="auto", id="auto-id"):
            raise AutoResumeTestError()
    with tracklab.init(mode="offline", resume="auto", id="explicit") as run2:
        pass
