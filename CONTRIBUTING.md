...
We at TrackLab ❤️ open source and welcome contributions from the community!
This guide discusses the development workflow of the `tracklab` library.
...
1.  Browse the existing [Issues](https://github.com/tracklab/tracklab/issues) on GitHub to see
    if the feature/bug you are willing to add/fix has already been requested/reported.

    - If not, please create a [new issue](https://github.com/tracklab/tracklab/issues/new/choose).
...
2.  If you are a first-time contributor, please go to
    [`https://github.com/tracklab/tracklab`](https://github.com/tracklab/tracklab)
    ...
    - Clone the forked project to your machine and add the upstream repository
      that will point to the main `tracklab` project:

      ```shell
      git clone https://github.com/<your-username>/tracklab.git
      cd tracklab
      git remote add upstream https://github.com/tracklab/tracklab.git
      ```
...
At TrackLab, we ask that all PR titles conform to the
...
The TrackLab SDK is implemented in Python with a FastAPI backend.
...
Install Node.js version 18+ following the instructions [here](https://nodejs.org/) or using your package manager:
```shell
# macOS
brew install node
...
We recommend installing the `tracklab` package in the editable mode with either `pip` or `uv`:

```shell
uv pip install -e .
```

For frontend development, install the Node.js dependencies:
```shell
cd frontend
npm install
```
