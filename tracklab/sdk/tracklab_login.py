...
import tracklab
from tracklab.errors import AuthenticationError, UsageError
from tracklab.old.settings import Settings as OldSettings
...
def _handle_host_wandb_setting(host: Optional[str], cloud: bool = False) -> None:
    ...
    This takes the parameter from tracklab.login or wandb login for use by the
    application's APIs.
    """
    _api = InternalApi()
...
class _WandbLogin:
    def __init__(
        ...
        self._tracklab_setup = tracklab_setup.singleton()
        self._tracklab_setup.settings.update_from_dict(login_settings)
        self._settings = self._tracklab_setup.settings
    ...
    def _print_logged_in_message(self) -> None:
        ...
        username = self._tracklab_setup._get_username()
        ...
        if username:
            ...
            entity = self._settings.entity or self._tracklab_setup._get_entity()

            entity_str = ""
            # check if entity exist, valid (is part of a certain team) and different from the username
            if (
                entity
                and entity in self._tracklab_setup._get_teams()
                and entity != username
            ):
    ...
    def update_session(
        ...
        # from server.
        if not self._tracklab_setup.settings._offline:
            self._tracklab_setup._update_user_settings()
