...
class AgentProcess:
    ...
    def __init__(
        ...
        if command:
            ...
            if env.get(tracklab.env.SERVICE):
                env.pop(tracklab.env.SERVICE)
    ...
    def _start(self, finished_q, env, function, run_id, in_jupyter):
        ...
        # call user function
        tracklab.termlog(f"Agent Started Run: {run_id}")
        if function:
            function()
        tracklab.termlog(f"Agent Finished Run: {run_id}\n")
...
class Agent:
    ...
    def __init__(
        ...
        self._report_interval = tracklab.env.get_agent_report_interval(
            self.REPORT_INTERVAL
        )
        self._kill_delay = tracklab.env.get_agent_kill_delay(self.KILL_DELAY)
        self._finished = 0
        self._failed = 0
        self._count = count
        self._sweep_command = []
    ...
    def is_flapping(self):
        ...
        if os.getenv(tracklab.env.AGENT_DISABLE_FLAPPING) == "true":
            return False
        if time.time() < tracklab.START_TIME + self.FLAPPING_MAX_SECONDS:
        ...
        try:
            ...
            try:
                if not self._in_jupyter:
                    tracklab.termlog("Terminating and syncing runs. Press ctrl-c to kill.")
                for _, run_process in self._run_processes.items():
                    try:
                ...
                tracklab.termlog("Killing runs and quitting.")
                for _, run_process in self._run_processes.items():
                    try:
                        run_process.kill()
                    except OSError:
    ...
    def _command_run(self, command):
        ...
        sweep_id = os.environ.get(tracklab.env.SWEEP_ID)
