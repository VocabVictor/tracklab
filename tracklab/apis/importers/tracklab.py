...
import requests
import urllib3
import tracklab_workspaces.reports.v1 as wr
import yaml
from tracklab_gql import gql
from tracklab_workspaces.reports.v1 import Report

import tracklab
from tracklab.apis.public import ArtifactCollection, Run
from tracklab.apis.public.files import File
from tracklab.util import coalesce, remove_keys_with_none_values
...
class WandbRun:
    ...
    def runtime(self) -> Optional[int]:
        tracklab_runtime = self.run.summary.get("_wandb", {}).get("runtime")
        base_runtime = self.run.summary.get("_runtime")

        if (t := coalesce(tracklab_runtime, base_runtime)) is None:
            return t
        return int(t)
        ...
        *,
        ...
        for i, group in enumerate(groups_of_artifacts, 1):
            ...
            if art.description == ART_SEQUENCE_DUMMY_PLACEHOLDER:
                ...
                if tracklab_run is None:
                    logger.warning(
                        f"Run for {art.name=} does not exist (deleted?), using {run_or_dummy=}"
                    )
                    tracklab_run = run_or_dummy

                new_art = _clone_art(art)
                group = [new_art]
                run = WandbRun(tracklab_run, **self.run_api_kwargs)
