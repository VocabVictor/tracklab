...
class WandbObserver(RunObserver):
    ...
    Args:
        Accepts all the arguments accepted by tracklab.init().
        ...
        reinit — Shorthand for the reinit setting that defines what to do when `tracklab.init()` is called while a run is active. See the setting's documentation.
        ...
        allow_val_change — whether to allow tracklab.config values to change, by default we throw an exception if config values are overwritten. (default: False)
        ...
        if result:
            ...
            for i, r in enumerate(result):
                if isinstance(r, float) or isinstance(r, int):
                    tracklab.log({f"result_{i}": float(r)})
        ...
        for metric_name, metric_ptr in metrics_by_name.items():
            for _step, value in zip(metric_ptr["steps"], metric_ptr["values"]):
                if isinstance(value, numpy.ndarray):
                    tracklab.log({metric_name: tracklab.Image(value)})
                else:
                    tracklab.log({metric_name: value})
