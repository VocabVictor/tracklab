...
from tracklab.data_types import Node
...
class TorchHistory:
    ...
    def __init__(self):
        global torch
        torch = tracklab.util.get_module("torch", "Could not import torch")
        self._hook_handles = {}
        self._num_bins = 64
    ...
    def log_tensor_stats(self, tensor, name):  # noqa: C901
        ...
        tracklab.run._log(
            {name: tracklab.Histogram(np_histogram=(tensor.tolist(), bins.tolist()))},
            commit=False,
        )
...
class TorchGraph(tracklab.data_types.Graph):
    def __init__(self):
    ...
    def hook_torch(cls, model, criterion=None, graph_idx=0):
        tracklab.termlog("logging graph, to disable use `tracklab.watch(log_graph=False)`")
        graph = TorchGraph()
    ...
    def create_forward_hook(self, name, graph_idx):
        ...
        def after_forward_hook(module, input, output):
            ...
            if not self._graph_hooks:
                ...
                tracklab.run.summary[f"graph_{graph_idx}"] = self
    ...
    def hook_torch_modules(
        ...
        layers = 0
        graph = self
        if hasattr(module, "_tracklab_watch_called") and module._tracklab_watch_called:
            raise ValueError(
                "You can only call `tracklab.watch` once per model.  Pass a new instance of the model if you need to call tracklab.watch again in your code."
            )
        module._tracklab_watch_called = True
        if criterion:
        ...
        for name, sub_module in module.named_children():
            ...
            if isinstance(sub_module, tuple(module_types)):
                ...
                try:
                    ...
                    tracklab.run._torch._hook_handles[
