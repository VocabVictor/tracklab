...
impl GpuMonitors {
    ...
    pub fn environment_metadata(&self) -> HashMap<String, serde_json::Value> {
        let mut env_metadata = HashMap::new();
        
        // Add basic GPU information
        let gpu_count = self.monitors.len();
        env_metadata.insert("gpu_count".to_string(), serde_json::json!(gpu_count));
        
        // Determine GPU type based on available monitors
        let gpu_type = if self.monitors.is_empty() {
            "none"
        } else {
            // This is a simplified version - in production you'd want more detail
            "mixed"
        };
        env_metadata.insert("gpu_type".to_string(), serde_json::json!(gpu_type));
        
        let mut environment = HashMap::new();
        environment.insert("gpu_count".to_string(), serde_json::json!(gpu_count));
        environment.insert("gpu_type".to_string(), serde_json::json!(gpu_type));
        
        env_metadata.insert("environment".to_string(), serde_json::json!(environment));
        env_metadata
    }

    pub fn sample(&self) -> HashMap<String, metrics::MetricValue> {
        // This is a simplified synchronous wrapper for the async collect_metrics
        // In production, this should be properly async
        let rt = tokio::runtime::Handle::current();
        let metrics = rt.block_on(self.collect_metrics(0, None));
